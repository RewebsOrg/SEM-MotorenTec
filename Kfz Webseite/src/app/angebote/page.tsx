...
// --- TYPE DEFINITIONS ---
// Defines the structure of an offer item.
// This interface matches the data structure we expect from our API.
interface Offer {
    id: string;
    title: string;
    price: string;
...
// --- DUMMY DATA ---
// Replace this with an empty array [] when connecting the real API.
// These items simulate the response from the eBay API for development and styling.
const DUMMY_OFFERS: Offer[] = [
    {
        id: '1',
        title: 'BMW N47D20C Austauschmotor Generalüberholt',
        price: '2.499,00 EUR',
        image: '/images/engine-placeholder-1.jpg', // Ensure these placeholders exist or use a generic one
...
export default function AngebotePage() {
    const [offers, setOffers] = useState<Offer[]>(DUMMY_OFFERS); // Initialize with Dummy Data
    const [loading, setLoading] = useState(false); // Set to true if you fetch on mount
    const [error, setError] = useState<string | null>(null);

    // --- API FETCH FUNCTION ---
    // This function is prepared to fetch real data from your Next.js API route.
    // To activate:
    // 1. Rename DUMMY_OFFERS to empty array [] or null.
    // 2. Uncomment the fetch code inside this function.
    // 3. Call fetchEbayOffers() inside a useEffect hook.
    const fetchEbayOffers = async () => {
        ...
        try {
          setLoading(true);
          const res = await fetch('/api/fetchEbayOffers');
    ...
    // Currently we just log that we would fetch data.
    useEffect(() => {
        fetchEbayOffers();
    }, []);
                            ...
                            {offers.map((offer, index) => (
                                ...
                                >
                                    ...
                                    <div className="relative aspect-[4/3] bg-anthracite-dark overflow-hidden">
                                        {/* Placeholder Logic: In a real app, use next/image */}
                                        <img
                                            src={offer.image}
                                            alt={offer.title}
                                    ...
                                    <div className="p-6 flex flex-col flex-grow">
                                        ...
                                        <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
                                            ...
                                            {/* Button: "Zu eBay" - This is the only clickable action for the external link */}
                                            <a
                                                href={offer.link}
                                                target="_blank"
